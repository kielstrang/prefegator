...
module.exports = (knex) => {
  ...
  function saveBallot(url, ballot) {
    return _getPollIDByURL(url)
      .then(pollID => {
        return _writeBallot(pollID, ballot) //ballot.votes & ballot.votername?
  ...
  function _getPollBallotCount(pollID) {
    return knex('ballots')
      .select('id').where('poll_id', pollID)
      .then(ballots => ballots.length);
  }
  ...
  function _buildPollObject([{ name, email, desc, url }, options, voteCount]) {
    console.log('Vote Count:', voteCount);
    const poll = { name, email, desc, url, voteCount };
    poll.options = [];
